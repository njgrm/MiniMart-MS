  ...
  (
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        if (setOpenProp) {
          const openState = typeof value === "function" ? value(openProp ?? defaultOpen) : value
          setOpenProp(openState)
        } else {
          _setOpen(value)
        }
      },
      [setOpenProp, openProp, defaultOpen]
    )
    ...
    // Calculate the current sidebar width based on state
    const currentSidebarWidth = open ? SIDEBAR_WIDTH : SIDEBAR_WIDTH_ICON
        ...
        <div
          className={cn(
            ...
            variant === "floating" || variant === "inset"
              ? "p-2"
              : "",
            // Apply border directly to the fixed positioned container for full height coverage
