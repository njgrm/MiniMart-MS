...
export default function DemoTerminal() {
  return (
    <Terminal>
      <Terminal.Command>npm install @repo/terminal</Terminal.Command>
      <Terminal.Output>Installing dependencies...</Terminal.Output>
      <Terminal.Command>npm start</Terminal.Command>
      <Terminal.Cursor />
    </Terminal>
  )
}
...
export default function AppDock() {
  return (
    <Dock className="fixed bottom-4 left-1/2 -translate-x-1/2">
      ...
      <DockIcon icon={User} label="Profile" href="/profile" />
      <DockIcon icon={Mail} label="Messages" href="/messages" />
...
export default function MarketShareChart() {
  ...
  return (
    <PieChart
      ...
      outerRadius={100}
      showLabel={true}
      labelFormat={(entry) => `${entry.name}: ${entry.value}`}
    />
  )
}
```
...
export default function InteractiveButton() {
  return (
    <Magnetic strength={0.3} radius={100}>
      <button className="px-8 py-4 bg-primary text-primary-foreground rounded-lg">
        Hover me
      </button>
    </Magnetic>
  )
...
export default function Hero() {
  return (
    <ShimmeringText
      ...
      shimmerColor="#ffffff"
      shimmerDuration={2}
      className="text-5xl font-bold bg-gradient-to-r from-purple-600 to-pink-600"
    />
  )
}
```
...
export default function TogglePanel() {
  ...
  return (
    <>
      ...
      {modal.value && (
        <div className="modal">
          <p>Status: {loading.value ? "Saving..." : "Ready"}</p>
...
export default function CartCounter() {
  ...
  return (
    <div className="flex items-center gap-4">
      <button onClick={quantity.decrement} disabled={quantity.value === 0}>
        -
      </button>
      <span className="text-xl font-semibold">{quantity.value}</span>
      <button onClick={quantity.increment} disabled={quantity.value === 99}>
        +
      </button>
      <button onClick={quantity.reset} className="text-sm">
        Reset
      </button>
    </div>
  )
...
export default function ImagePreview() {
  ...
  return (
    <div className="relative">
      ...
      />
      {isHovering && (
        <div className="absolute top-0 left-full ml-4 z-10">
          <img src="/full-size.jpg" alt="Full size" className="w-64 h-64" />
        </div>
      )}
    </div>
  )
...
export default function OTPTimer() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-2xl font-mono">{countdown.seconds}s</p>
